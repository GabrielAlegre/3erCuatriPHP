...
class Usuario
{
    ...
    public static function TraerTodoLosUsuariosDeLaBaseDeDatos()
	{
        ...
        foreach ($arrayDeUsuariosRecuperados as $unUsuarioDeLaBaseDeDatos ) {
            array_push($usuarios, Usuario::crearUsuario($unUsuarioDeLaBaseDeDatos));
        } 
        return $usuarios;
	}
    ...
    public function intentarCambiarClaveUsuario($claveNueva)
    {
        ...
        foreach ($usuariosDelaBaseDeDatos as $usuarioRegistradoEnLaBaseDeDatos) {
            ...
            {
                if(Usuario::updateClaveUsuario($claveNueva, $this->password, $this->mailUser))
                {
                    return $retornoInformativo="Se cambio la clave del usuario correctamente\n";
                }
                else
                {
                    return $retornoInformativo="ERROR. No se cambio la clave del usuario\n";
                }
            ...
            {
                return "No se realizo el cambio de clave ya que la contraseña enviada no coincide con el mail\n";
                
            }
            else if($this->mailUser != trim($usuarioRegistradoEnLaBaseDeDatos->mailUser))
            {
                $retornoInformativo="Usuario no registrado, no coincide el mail con ningun usuario de la base de datos\n";
            }
        }
    ...
    public static function updateClaveUsuario($nuevaClave, $claveVieja, $mail)
    {
        $objetoAccesoDato = AccesoDatos::dameUnObjetoAcceso(); 
        
        $consulta =$objetoAccesoDato->RealizarConsulta("UPDATE usuarios SET usuarios.CLAVE = '$nuevaClave'
                                                        WHERE usuarios.CLAVE='$claveVieja' AND usuarios.MAIL='$mail'");    
